...
There is a couple additional things we can fix. 

We can simplify the feed by just including all users in the fanout and only rendering the users feed which will include both 
all users posts and their own posts. 

We can also leverage bootstrap and CSS to style the elements so it looks like a better feed

Finally we can use local storage and session store in Vue to ensure that page refreshes don't clear the state.

The updated vue file after these updates is:
  ...
  methods: {
     async login() {
       try {
         ...
         // allow feed to get updated and sleep for 5s
...
After spinning up the app we can now enter a post and see the feed item get populated after the 5 second timeout.  
